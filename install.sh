...
if [ -z "$1" ]; then
    ...
    elif [ "$ARGUMENT" == "main" ]; then
        PYTHON_SCRIPT="python3 BotMEXC_2.0/strategies/envelopes/main.py"
    else
    ...
    if [ -n "$PYTHON_SCRIPT" ]; then
        ...
        if grep -Fxq "$PYTHON_SCRIPT" BotMEXC_2.0/1hcron.sh; then
            echo "Le script $PYTHON_SCRIPT existe déjà dans 1hcron.sh"
        else
            # Ajouter la ligne au fichier 1hcron.sh
            echo "$PYTHON_SCRIPT" >> BotMEXC_2.0/1hcron.sh
...
crontab -l | grep -q 'bash ./BotMEXC_2.0/1hcron.sh'
if [ $? -ne 0 ]; then
    # Ajouter la tâche cron
    (crontab -l 2>/dev/null; echo "0 * * * * /bin/bash ./BotMEXC_2.0/1hcron.sh >> cronlog.log") | crontab -
